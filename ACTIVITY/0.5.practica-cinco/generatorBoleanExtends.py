...
def process_and_tokenize_documents(corpus_dir, steams_dir, output_dir):
    ...
    for idx, filename in enumerate(os.listdir(corpus_dir), start=1):
        ...
        # Contar la cantidad de veces que aparecen los steams en el documento
        token_counts = {token: tokens.count(token) for token in steams if token in tokens}

        # Almacenar los conteos de tokens en el diccionario de resultados
        results[f"D{idx}"] = token_counts if token_counts else {None: 0}  # Si no hay steams, poner 0

        print(f"Procesado archivo {filename} con {len(token_counts)} steams encontrados.")

    # Crear un DataFrame con los conteos de steams en cada documento
    ...
    if 'Steams' in df_steams.columns:
        ...
        # Para cada término único, contar su aparición en cada documento
        for term in termino['Steams']:
            row = {'Termino': term}
            for col in results_df.columns:
                row[col] = results_df[col].get(term, 0)  # Si el término no existe en un documento, poner 0
            count_matrix.append(row)

        # Convertir la matriz de conteos en un DataFrame
        count_df = pd.DataFrame(count_matrix)

        # Rellenar los valores NaN con ceros
        results_df = count_df.fillna(0)
        
        # Concatenar los DataFrames
        final_df = pd.concat([termino, results_df.drop(columns='Termino', errors='ignore')], axis=1)
    else:
        print("La columna 'Steams' no existe en el archivo de steams procesados.")
        return

    # Guardar el DataFrame en un archivo Excel
    output_file_path = os.path.join(output_dir, '7MatrizFrecuenciasDeSteams.xlsx')
