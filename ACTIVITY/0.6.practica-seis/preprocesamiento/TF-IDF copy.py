...
def process_documents(app, vectorial=False):
    corpus_dir = 'corpus'
    ...
    if os.path.exists(corpus_dir):
        ...
        # Dependiendo de si es vectorial o estándar, usamos el valor de la entrada correspondiente
        if vectorial:
            query = app.search_entry_vectorial.get()
        else:
            query = app.search_entry_standard.get()

        # Asegurarse de que haya una consulta válida
        if query:
            ...
            # Mostrar resultados en la interfaz (en lugar de solo imprimir en consola)
            app.result_label.config(text=f"Consulta realizada: {query}\nResultados:\n{results_df}")
        else:
            app.result_label.config(text="Por favor, introduce una consulta.")
    else:
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Modelo Vectorial")
    ...
    def create_widgets(self):
        ...
        # Marco para la búsqueda estándar
        search_frame = tk.Frame(self.root, bg='black')
        search_frame.pack(pady=20)
        """
        # Campo de entrada de búsqueda estándar
        self.search_entry_standard = ttk.Entry(search_frame, width=30, style="TEntry")
        self.search_entry_standard.pack(side=tk.LEFT, padx=5)

        # Botón de búsqueda estándar
        search_button = ttk.Button(search_frame, text="🔍", style="TButton", command=lambda: process_documents(self, vectorial=False))
        search_button.pack(side=tk.LEFT)

        """
        
        
        # Marco para la búsqueda de proceso vectorial
        search_vectorial_frame = tk.Frame(self.root, bg='black')
        search_vectorial_frame.pack(pady=20)

        # Campo de entrada de búsqueda de proceso vectorial
        self.search_entry_vectorial = ttk.Entry(search_vectorial_frame, width=30, style="TEntry")
        self.search_entry_vectorial.pack(side=tk.LEFT, padx=5)
        
        # Botón para el preprocesamiento de documentos (Proceso vectorial)
        preprocess_button = ttk.Button(search_vectorial_frame, text="Proceso vectorial", command=lambda: process_documents(self, vectorial=True), style="TButton")
        preprocess_button.pack(side=tk.LEFT, padx=5)
        
        """
        
        # Botón para la indexación de los steams
        index_button = ttk.Button(self.root, text="Indexación", style="TButton")
        index_button.pack(pady=10)

        # Botón para cargar los steams en el Árbol B
        load_button = ttk.Button(self.root, text="Cargar STEAMS", style="TButton")
        load_button.pack(pady=10)
        """

        # Etiqueta para mostrar los resultados de la búsqueda
        self.result_label = tk.Label(self.root, text="", fg="white", bg="black", font=("arial", 16))
        self.result_label.pack(pady=10)

        # Canvas para mostrar los documentos del corpus
        ...
        # Mostrar los documentos en el Canvas
        ...
        # Desactivar pantalla completa con la tecla Escape
